...
PLL_Init_step3loop
    ...
    ;    Q, N, MINT, e MFRAC para configurar as configura��es desejadas da frequ�ncia de VCO.
    ;    ************
...
SYSCLK EQU (FXTAL/(Q+1)/(N+1))*(MINT+MFRAC/1024)/(PSYSDIV+1)
    LDR R1, =SYSCTL_PLLFREQ0_R                 ; R1 = SYSCTL_PLLFREQ0_R (ponteir    LDR R0, [R1]                               ; R0 = [R1] (valor)
